...
def get_tabs(categories):
    ...
    for index, tab in enumerate(categorical_data, start=1):
        static_name = FIELD_NAME_MAPPING.get(tab, frappe.unscrub(tab))  # Use static name if available
        tab_values[f"tab_{index + 1}_title"] = static_name
        # pre-render cards for each tab
        tab_values[f"tab_{index + 1}_content"] = frappe.render_template(
            "kersten/www/shop-by-category/category_card_section.html",
...
def get_category_records(categories):
    ...
    for category in categories:
        if category == "item_group":
            categorical_data["item_group"] = frappe.db.sql(f"""
                SELECT 
                    DISTINCT wi.name, wi.parent_website_itemgroup, wi.is_group, wi.route, wi.image
                FROM
