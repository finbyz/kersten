...
class ProductFiltersBuilder:
	def __init__(self, item_group=None):
		if not item_group:
			...
			self.doc = frappe.get_doc("Website Itemgroup", item_group)
	...
	def get_field_filters(self):
		...
		for df in fields:
			...
			if df.fieldtype == "Link":
				if self.item_group:
					include_child = frappe.db.get_value("Website Itemgroup", self.item_group, "include_descendants")
					if include_child:
						...
						item_or_filters.extend(
							[
								["Website Item Group", "website_itemgroup", "=", self.item_group],  # consider website item groups
							]
						)
					else:
						item_or_filters.extend(
